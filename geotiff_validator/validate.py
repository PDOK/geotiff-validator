...
def get_validations_for_validating_process(required_validations: str, recommended_validations: str) -> (list, list):
    ...
    if required_validations == "" and recommended_validations == "":
        required_validators = get_default_validators()
    else:
        ...
        validator_map = get_validator_map()
...
def append_validations_for_file(file_path, validation_results, required_validators, recommended_validators):
    success = True
...
def validate(geotiff_path, folder_path, required_validations: str="", recommended_validations: str=""):
    success = True

    required_validators, recommended_validators = get_validations_for_validating_process(required_validations, recommended_validations)
    validation_results = []

    if geotiff_path is not None:
        success = success and append_validations_for_file(geotiff_path, validation_results, required_validators, recommended_validators)
    else:
        ...
        for filename in dir_list:
            if utils.file_has_tiff_extension(filename):
                ...
                if not file_path.endswith("/"):
                    file_path += "/"
                file_path += filename
                success = success and append_validations_for_file(file_path, validation_results, required_validators, recommended_validators)

    return validation_results, required_validators, recommended_validators, success

def get_default_validators():
    return get_validator_classes()
...
def get_validator_map():
    return {x.code: x for x in get_validator_classes()}
